...
func spawnEnemy():
	...
	var enemySpawnTilePosition: Vector2 = getRandomEnemySpawnPosition()
	GlobalSignal.spawnEnemyOnTile.emit(enemySpawnTilePosition.x, enemySpawnTilePosition.y)
	tileToSpawnEnemiesArray.remove_at(0)


func getRandomEnemySpawnPosition() -> Vector2:
	# these need to be changed manually if you ever change the board size
	var randomXPosition = randi_range(0, 8)
	var randomYPosition = randi_range(0, 4)
	return Vector2(randomXPosition, randomYPosition)
#endregion
